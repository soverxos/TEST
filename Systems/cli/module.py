...
def list_modules_cmd():
    ...
    for idx, module_info in enumerate(sorted_modules):
        ...
        # Добавляем горизонтальный разделитель между модулями (кроме последнего)
        if idx < len(sorted_modules) - 1:
            # Используем правильные символы разделителя таблицы из box.SIMPLE
            # box.SIMPLE использует "─" для горизонтальных линий
            separator_char = "─"
            table.add_row(
                separator_char * 20,
                separator_char * 25,
