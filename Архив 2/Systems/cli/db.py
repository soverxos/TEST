...
@db_app.command(
    name="migrate-sqlite",
    help="📦 Мигрирует данные из SQLite в PostgreSQL/MySQL по текущему DSN.",
)
def db_migrate_sqlite_to_postgres(
    source_sqlite: Optional[Path] = typer.Option(
        ...
        "--source",
        "-s",
        help="Путь к файлу SQLite (по умолчанию берётся из настроек ядра).",
    ),
    target_dsn: Optional[str] = typer.Option(
        None,
        "--target",
        "-t",
        help="DSN PostgreSQL (по умолчанию берётся из db.pg_dsn).",
    ),
    confirm: bool = typer.Option(
        False,
        "--confirm",
        "-y",
        help="Подтвердить миграцию без лишнего вопроса.",
    ),
):
    """Копирует все строки из SQLite в PostgreSQL, сохраняя таблицы и данные."""
    console.print(Panel("[bold cyan]Миграция SQLite → PostgreSQL[/]", expand=False))
    ...
    if not sqlite_path.exists():
        console.print(f"[bold red]SQLite-файл не найден: {sqlite_path}[/bold red]")
        raise typer.Exit(code=1)
    ...
    if not target_dsn_final:
        if settings.db.type == "postgresql" and settings.db.pg_dsn:
            ...
            console.print(
                "[bold red]DSN для PostgreSQL/MySQL не указан ни в --target, ни в конфиге (db.pg_dsn или db.mysql_dsn).[/bold red]"
            )
            raise typer.Exit(code=1)
    else:
        if "postgresql" in target_dsn_final:
            ...
            console.print(
                "[bold red]Неподдерживаемый тип target DSN. Поддерживаются postgresql/mysql.[/bold red]"
            )
            raise typer.Exit(code=1)
    ...
    if not confirm:
        ...
        if not typer.confirm(
            f"Миграция перезапишет {target_label} ({target_dsn_final}). Продолжить?",
            abort=True,
        ):
            return

    console.print(f"[green]Источник:[/green] {sqlite_path}")
    console.print(f"[green]Цель:[/green] {target_dsn_final}")
    ...
    try:
        ...
        console.print(f"[bold red]Ошибка отражения SQLite-схемы: {e_reflect}[/bold red]")
        raise typer.Exit(code=1)

    if not src_meta.tables:
        console.print("[yellow]SQLite пуст (нет таблиц). Нечего переносить.[/yellow]")
        return
    ...
    console.print("[cyan]Создаём таблицы в PostgreSQL (если отсутствуют)…[/cyan]")
    try:
        dst_meta.create_all(bind=dst_engine, checkfirst=True)
    except Exception as e_create:
        console.print(f"[bold red]Ошибка создания таблиц: {e_create}[/bold red]")
        raise typer.Exit(code=1)

    console.print("[cyan]Копируем данные…[/cyan]")
    with src_engine.connect() as src_conn, dst_engine.begin() as dst_conn:
        for table in src_meta.sorted_tables:
            ...
            if dst_table is None:
                console.print(f"[yellow]Таблица {table.name} отсутствует в PostgreSQL — пропускаем.[/yellow]")
                continue
            dst_conn.execute(dst_table.insert(), rows)
            console.print(f"[green]Перенесено {len(rows)} строк в {table.name}[/green]")

    console.print("[bold green]Миграция завершена.[/bold green]")
    console.print(
        "[dim]Теперь перезапустите бота и при необходимости выполните 'sdb db upgrade head'.[/dim]"
    )
