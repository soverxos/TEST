...
class ModuleAction(CallbackData, prefix=f"{CORE_CALLBACK_PREFIX}_module_action"):
    """Callback data для действий модулей (команд)"""
    module_name: str
    command: str  # Имя команды из манифеста
    action: Optional[str] = None  # Дополнительное действие (например, "execute", "show_help")
