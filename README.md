...
Если вы только начинаете работать с SwiftDevBot, этот раздел поможет вам быстро создать свой первый модуль и понять основные концепции.

### 🚀 Создание первого модуля "Hello World"

Давайте создадим простейший модуль, который отвечает на команду `/hello`:
...
Эта команда создаст структуру модуля в `Modules/hello_world/`.

#### Шаг 2: Редактирование модуля

Откройте файл `Modules/hello_world/__init__.py` и замените его содержимое на:
...
async def setup_module(dp, bot, services):
    """Функция настройки модуля - вызывается при загрузке"""
    dp.include_router(router)
    print("✅ Модуль hello_world загружен!")
```
...
Отправьте боту команду `/hello` в Telegram. Бот должен ответить "Привет, мир! 👋"

### 📚 Основные концепции (упрощенно)

#### 1. **DI (Dependency Injection) - Внедрение зависимостей**
Вместо создания объектов напрямую, мы получаем их "извне". Это делает код более гибким и тестируемым.

**Пример:** Вместо `db = Database()` мы получаем `db` через параметр функции:
```python
async def my_handler(message: types.Message, db: Database):
    # db уже готов к использованию
    users = await db.get_users()
```

#### 2. **Pydantic - Валидация данных**
Pydantic проверяет, что данные соответствуют ожидаемому формату.
...
# Автоматически проверит, что name - строка, age - число
user = User(name="Иван", age=25)
```

#### 3. **Alembic - Миграции базы данных**
Alembic помогает изменять структуру базы данных безопасно.

**Основные команды:**
```bash
python3 sdb.py db migrate    # Применить изменения
python3 sdb.py db upgrade    # Обновить схему БД
```

**Что это значит:** Когда вы добавляете новую таблицу или поле в модели, Alembic создает скрипт миграции, который безопасно обновит вашу базу данных.

#### 4. **React (для веб-панели)**
React - это библиотека для создания интерфейсов. В SwiftDevBot она используется для веб-панели управления.

**Для начала работы с ботом React не обязателен** - вы можете создавать модули только для Telegram бота.

### 🎯 Следующие шаги

1. **Изучите примеры модулей:**
   - `Modules/example_module/` - полный пример с базой данных
   - `Modules/sys_status/` - системный модуль

2. **Прочитайте документацию:**
   - [Создание модуля](#-модули) - подробное руководство
   - [API документация](#-api-документация) - справочник по классам

3. **Экспериментируйте:**
   - Добавьте больше команд в свой модуль
   - Попробуйте работать с базой данных
   - Создайте интерактивные диалоги

### ❓ Частые вопросы

**Q: Нужно ли знать все эти технологии сразу?**  
A: Нет! Начните с простых модулей без базы данных. Изучайте по мере необходимости.

**Q: Что делать, если что-то не работает?**  
A: Проверьте логи: `python3 sdb.py monitor logs`. Большинство ошибок там видны.

**Q: Как отладить модуль?**  
A: Запустите бота с флагом `--debug`: `python3 sdb.py run --debug`
...
# Перезагрузить модуль (выполняет полный рестарт процесса бота)
...
python3 sdb.py module reload name     # Перезагрузить модуль (полный рестарт бота)
...
# Запуск тестов
python3 -m pytest
# Специальный набор для веб API (использует `.venv` и реальные маршруты)
make web-test
...
Веб-панель можно настроить через переменные окружения в `.env` файле:

```env
# Настройки веб-панели
SDB_WEB_HOST="0.0.0.0"        # Хост (0.0.0.0 для доступа со всех интерфейсов)
SDB_WEB_PORT=8000              # Порт
SDB_WEB_RELOAD=false           # Автоперезагрузка (для разработки)
SDB_WEB_WORKERS=1              # Количество воркеров
SDB_WEB_TOKEN_LIFETIME_MINUTES=60  # Время жизни JWT токена
```

После настройки `.env` просто запустите:
```bash
python3 sdb.py web start
```

Все параметры будут взяты из `.env`. Параметры командной строки (`--host`, `--port` и т.д.) имеют приоритет над переменными окружения.

#### API Endpoints

- `GET /api/users` — возвращает список пользователей с ролями, аватарками и флагом блокировки.
- `POST /api/users/{id}/block` — принимает `{ "block": true|false }`, переключает `is_bot_blocked` в БД и возвращает актуальный статус.
- `GET /api/modules` — предоставляет информацию о модулях (имя, статус, описание, версия, принадлежность к системным).
- `POST /api/modules/{name}/toggle` — включает/выключает плагин и обновляет `enabled_modules.json`, сохраняя конфигурацию для следующего запуска.
- Для локальной проверки вызовов используется `make web-test`: команда активирует `.venv` и прогоняет `tests/test_web_app.py`, убеждаясь, что маршруты работают с заглушками `DummyServices`.

## 📖 Полная инструкция по платформе
...
2. Установите зависимости:
   ...
   pip install -r requirements.txt
   ```
   Для разработки и тестов запускайте `make dev`, он добавит `pytest`, `pytest-asyncio`, `pylint`, `black`, `isort`, `mypy`.
3. Создайте `.env` на базе `env.example` и заполните ключи:
   - `BOT_TOKEN` (от @BotFather)
   - `SDB_CORE_SUPER_ADMINS` (список Telegram ID)
   - `SDB_DB_TYPE` + параметры для SQLite/PostgreSQL/MySQL
   - `SDB_CACHE_TYPE` (`memory` или `redis`)
   - `SDB_WEB_HOST` и `SDB_WEB_PORT` (для веб-панели, по умолчанию `127.0.0.1:8000`)
   - Опционально: параметры безопасности (`SDB_SECURITY_LEVEL`, `SDB_SECURITY_SIGNATURE_REQUIRED`, `SDB_SECURITY_SANDBOX_ENABLED`)

### 🛠️ Общие команды CLI

- `python3 sdb.py run` / `make run` — старт бота (с номинальными логами).
- `make run-verbose` / `make run-debug` — запуск с подробными логами.
- `python3 sdb.py status` — информация о процессе (PID, uptime).
- `python3 sdb.py stop` / `restart` — управление процессом.
- `python3 sdb.py db init/migrate/upgrade` — управление миграциями.
- `python3 sdb.py module list|enable|disable|reload <module>` — работа с модулями.
- `python3 sdb.py user list|block|unblock|role <id>` — управление пользователями.
- `python3 sdb.py security keys|sign` — цифровые подписи и управление ключами.
- `python3 sdb.py config init` — интерактивный мастер, как альтернатива ручной настройке.
- `python3 sdb.py web start` — запуск web-панели (настройки из `.env`: `SDB_WEB_HOST`, `SDB_WEB_PORT` и т.д.).

Все команды протестированы `pytest tests/test_cli.py` — этот тест рекурсивно перебирает `cli_main_app.registered_groups` и вызывает `--help` для каждого пути. Запускайте его перед релизом, чтобы гарантировать, что структура CLI не сломалась:
```bash
...
- `make test` — прогоняются все тесты (`pytest tests/ -v`). Теперь использует `pytest-asyncio`, чтобы поддерживать асинхронные `TestHealthChecker`.
- `make web-test` — активирует `.venv` и запускает `tests/test_web_app.py` (FastAPI + DummySession).
- `pytest tests/test_web_app.py` — можно запускать вручную для фокусного тестирования API.
- `make test-cov` — тесты с отчетом по покрытию.
- CI (GitHub Actions) выполняет `make test` + `make web-test`, гарантируя, что `pytest-asyncio` и `httpx` установлены и что API стабильно работает.

### 🔄 Web-панель и API

1. Перейдите в директорию `Systems/web`.
2. Установите зависимости `npm install`.
3. Запустите веб-сервер `npm run dev` и авторизуйтесь по токену из `.env`.
4. Веб-компоненты `Users` и `Modules` используют новые API (блокировка/переключение модулей).
5. После изменения статуса пользователей/модулей нажмите `Обновить`, страницы автоматически загружают данные через `loadUsers/loadModules`.

### 📦 Разработка модулей

1. Создайте шаблон через `python3 sdb.py module create my_module`.
2. Определите `manifest.yaml` с `permissions`, `dependencies`, `version`.
3. Зарегистрируйте роутер в `setup_module`.
4. Активируйте модуль `python3 sdb.py module enable my_module`, затем `reload`.
5. Используйте `CryptoManager` для подписи: ключи храните в `project_data/Security/crypto_keys`.

### 🧭 Локализация и текстовый контент

1. Переводы лежат в `Systems/locales/{ru,en,ua}.yaml`.
2. Добавляйте ключи в `admin_texts`, `roles_texts`, `users_texts` и обновляйте `Systems/core/admin/...` обработчики.
3. В версиях ботов тексты читаются через `Translator` (i18n middleware); используйте `admin_texts.get("key")` вместо жесткого текста.

### 🧹 Поддержка качества

- `make clean` очищает `.pytest_cache`, `.mypy_cache`, `__pycache__`.
- `make lint` запускает `pylint` на `Systems/core`.
- `make format`/`make format-check` используют `black` и `isort`.
- `make type-check` — статическая проверка `mypy`.
- Документируйте новые команды и API в README/Docs и обновляйте CHANGELOG.md.

### 🧰 Инструменты CI

- `.github/workflows/ci.yml` устанавливает `.venv`, `pytest-asyncio`, `httpx`, выполняет `pytest tests/ -v` и `pytest tests/test_web_app.py -v`.
- `make web-test` интегрирован в CI, поэтому держите `tests/test_web_app.py` актуальными.
- Запускайте `make test` и `make web-test` локально перед пушем.

### 💬 Тестовые данные и фикстуры

- `tests/conftest.py` настраивает фикстуры FastAPI, database, telegram.
- `tests/test_cli.py` использует `monkeypatch` для `SDB_CLI_MODE`/`SDB_VERBOSE`.
- `tests/test_web_app.py` содержит `DummySession`, `DummyServices`, `ModuleLoader` мок.
- После добавления новых фикстур обновляйте `CONTRIBUTING` (если есть) и документацию.
